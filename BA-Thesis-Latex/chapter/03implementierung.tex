...
\subsection{1. YOLO Variante (Jeder Frame einzeln)} {
	...
	\lstinputlisting[linerange={59,64,71,73,76,83,84,86,87,93-97}, caption={Verkürzter Ausschnitt aus der \protect\lstinline|run_yolo| Funktion in yolo\_every\_frame.py}, label = {cd:run_yolo_func_in_yolo_every_frame.py}]{../Code/YOLO/yolo_every_frame.py}
...
\subsection{2. YOLO Variante (direkte Verarbeitung)}{
	...
	Wenn die Schleifen durchgelaufen sind, wird das veränderte \lstinline|result| Objekt und das \lstinline|options| Dictionary zurückgeben. \\
	Danach wird das Video an dem angegebenen Pfad gespeichert. Es werden statistische Auswertungen (siehe \ref{py:Shape_Sim_Meas}) durchgeführt und die Timestamps gespeichert. Damit ist das Programm abgeschlossen. \\
	}
